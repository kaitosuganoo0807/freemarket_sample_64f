class CategoryController < ApplicationController
  ...
  def category
    @categories = MainCategoty.includes(sub_categories: :sub2_categories).find_by(id: params[:id])
    category_id = @categories.sub_categories.map(&:sub2_categories).flatten
    category_id.collect!{|category| category[:id]}
    @items = Item.includes(:images).where(category: category_id).order("created_at DESC").page(params[:page]).per(10)
  end
  ...
  def category3
    ...
    @items = Item.includes(:images).where(category: @categories[:id]).order("created_at DESC").page(params[:page]).per(10)
  end
